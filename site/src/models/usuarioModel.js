...
function tentativa(idUsuario) {
    console.log("ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function entrar(): ",idUsuario)
    var instrucao = `
        select count(fkUsuario) as qtdtentativas from conhecimento where fkUsuario = ${idUsuario}; 
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
...
function cadastrar(nome, email, senha, estilo) {
    ...
    var instrucao = `
        INSERT INTO usuario (nome, email, senha, estiloPilotagem) VALUES ('${nome}', '${email}', '${senha}', '${estilo}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
...
function cadastrarConhecimento(pontuacao, idu) {
    ...
    //  e na ordem de inserção dos dados.
    var instrucao = `
        INSERT INTO conhecimento (fkUsuario, pontuacao) VALUES ('${idu}', '${pontuacao}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
